...
class Labelme2coco_keypoints():
    ...
    def _annotation(self, bboxes_list, keypoints_list, json_path, src_img_path):
        ...
        #     sys.exit()
        count_jyz = 0
        cut_path = '/home/guxiaowei/mmpose-master/data/show_label/'
        for object in bboxes_list:
            ...
            keypoint1, keypoint2 = keypoints_list[count_jyz * args.join_num], keypoints_list[count_jyz * args.join_num + 1]
            ...
            cv2.circle(src_img, (int(annotation['keypoints'][0]),int(annotation['keypoints'][1])), 2, (0,0,255), 2)
            cv2.circle(src_img, (int(annotation['keypoints'][3]),int(annotation['keypoints'][4])), 2, (0,255,255), 2)
...
if __name__ == '__main__':
    ...
    print('{} for training'.format(len(json_list_path)),
          '\n{} for testing'.format(len(json_list_path)))
    print('Start transform please wait ...')

    l2c_train = Labelme2coco_keypoints(args)  # 构造数据集生成类

    # 生成训练集
    train_keypoints = l2c_train.to_coco(json_list_path)
